...
const executeCommand = (command, args = [], type) => {
...
const executeCommandAsync = (command, args = []) => {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { stdio: 'inherit' });
...
const startProject = async () => {
  await downloadFile(url, outputFile)
    .then(() => executeCommand('npm', ['run', 'clean-api-docs', ['all']], 'Clean API Docs 🧹'))
    .then(() => executeCommand('npm', ['run', 'gen-api-docs', ['all']], 'Generate API Docs ⚙'))
...
app.get('/webhook', (req, res) => {
  ...
  downloadFile(url, outputFile)
    .then(() => executeCommandAsync('npm', ['run', 'clean-api-docs', ['all']], 'Clean API Docs 🧹'))
    .then(() => executeCommandAsync('npm', ['run', 'gen-api-docs', ['all']], 'Generate API Docs ⚙'))
